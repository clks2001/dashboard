...
function tableView(table, compId){
	...
	/**
	$("#T" + compId + " div.ctx").load("TableView.action", {"tableJson":tableJson, "kpiJson":kpiJson, "kpiType":kpiType, "compId":compId}, function(){
		//重新注册拖放事件(从度量拖入的事件)
		initDropDiv(compId);
	});
	**/
...
function searchDims(val, vls){
	...
	//tab.panel('refresh', 'DimFilter!search.action?dimId='+dimid+'&tid=' + comp.tid);
}
function filterDims(){
	var dimid = curTmpInfo.ckid;
	var compId = curTmpInfo.compId.replace("T", "");
...
function isExistDateDim(comp, tp){
	...
	if(tp == 'table'){
		...
		for(var i=0; i<comp.tableJson.cols.length; i++){
			var tp = comp.tableJson.cols[i].type;
		...
		for(var i=0; i<comp.tableJson.rows.length; i++){
			var tp = comp.tableJson.rows[i].type;
			if(tp == 'year' || tp == 'quarter' || tp == 'month' || tp == 'day'){
				ret = true;
				break;
			}
