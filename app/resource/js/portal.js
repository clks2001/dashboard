		...
		if(pageId == ''){ //未保存过，提示用户输入名称
			...
			buttons:[{
					...
					handler:function(){
						...
						$.post("PortalIndex!save.action", {pageInfo:JSON.stringify(pageInfo), pageName:name, "cataId":cata.id, "is3g":"y"}, function(resp){
							pageInfo.id = resp;
							msginfo("保存成功！","suc");
							//更新页面为未修改
			...
			$("#pdailog #reportcatalist").tree({
				url:'../m/MobReportType!tree.action',
				dnd:false,
			...
			$.post("PortalIndex!save.action", {"pageInfo": JSON.stringify(pageInfo), "pageId":pageId}, function(resp){
				msginfo("保存成功！", "suc");
				//更新页面为未修改
				curTmpInfo.isupdate = false;
				if(cb){
...
function printpage(){
	...
	var ctx = "<form name='prtff' method='post' target='printwindow' action=\"PortalIndex!print.action\" id='expff'><input type='hidden' name='pageInfo' id='ppageInfo'></form>";
...
function reloadDatasetTree(){
	$('#datasettree').tree({
		url:'PortalIndex!cubeTree.action?selectDsIds=' + (pageInfo.selectDs?pageInfo.selectDs:""),
		dnd:true,
		lines:true,
		onBeforeDrag:function(target){
...
function compevent(compId){
			...
			handler:function(){
				...
				if(idx == 0){
					...
					if(comp.type == "chart"){
						...
						if(comp.tableJson){
							comp.tableJson.link = {target:linkcompId, type:(linkComp==null?"":(linkComp.type == 'table' ? "cross": linkComp.type)), url:url, alias:sendCol};
						}else{
					...
					if(comp.type == "chart"){
						...
						if(comp.tableJson){
							comp.tableJson.linkAccept = {col:col, alias:alias, type:type, dftval: val, valType: valType};
						}else{
							msginfo("组件还未定义数据，不能定义事件。");
						}
					}
	...
	$("#compevent_tab #cleanAcceptEvent").bind("click", function(){
		...
		if(comp.type == "chart"){
			...
			delete comp.tableJson.linkAccept;
		}
	});
}
/**
