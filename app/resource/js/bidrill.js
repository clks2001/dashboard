...
function openTheDim(compId, pos, val, vdesc, oldDimId){
	var comp = findCompById(compId);
	var oldDim = findDimById(oldDimId, pos=="row"?comp.tableJson.rows:comp.tableJson.cols);
		...
		for(i=0; i<resp.length; i++){
			if(resp[i].dim_id == oldDimId){
				...
				if(i < resp.length - 1 && resp[i + 1].grouptype == resp[i].grouptype){
					var t = resp[i + 1];
					//判断下级维是否存在
					if(dimExist(t.dim_id, comp.tableJson.cols) || dimExist(t.dim_id, comp.tableJson.rows)){
						deal = 2;
						break
					}
