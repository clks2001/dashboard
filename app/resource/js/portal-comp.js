...
function editTableData(compId){
	...
	var comp = findCompById(compId);
	var str = "";
	for(var i=0; comp.tableJson&&comp.tableJson.rows&&i<comp.tableJson.rows.length; i++){
		str = str + "<span id=\"d_"+comp.tableJson.rows[i].id+"\" class=\"dimcol\"><span class=\"text\">"+comp.tableJson.rows[i].dimdesc+"</span><a style=\"opacity: 0.6;\" href=\"javascript:;\" onclick=\"setRdimInfo(this, "+comp.tableJson.rows[i].id+", '"+comp.tableJson.rows[i].dimdesc+"','"+compId+"')\"> &nbsp; </a></span>";
	}
	...
	var colstr = "";
	for(var i=0; comp.tableJson&&comp.tableJson.cols&&i<comp.tableJson.cols.length; i++){
...
function setTableProperty(comp){
	...
	$('#compSet').propertygrid({
		...
		data:[
			...
			{name:'锁定表头',col:'lockhead', value:(comp.tableJson&&comp.tableJson.lockhead?comp.tableJson.lockhead:""), group:'交叉表属性', editor:{
				type:"checkbox",
				options: {"on":true, "off":false}
			}},
			{name:'锁定后交叉表高度',col:'height', value:(comp.tableJson&&comp.tableJson.height?comp.tableJson.height:""), group:'交叉表属性', editor:"numberbox"},
...
function crossdrill(compId){
	var comp = findCompById(compId);
	if(comp.tableJson.rows.length != 1){
		msginfo("交叉表行标签只有一个维度的时候才能配置交叉表钻取。");
		return;
	}
	var dd;
	if(!comp.tableJson.drillDim){
		comp.tableJson.drillDim = [];
	}else{
		dd = comp.tableJson.drillDim[0];
	}
	var dims = null;
...
function tableView(table, compId){
	...
	**/
	...
	//处理参数
	var params = table.params ? JSON.stringify(table.params) : "[]";
...
function dimexchange(){
	...
	if(pos == 'col'){
		...
		if(comp.tableJson.cols.length == 0){
			$("#tableData #tabCols").remove();
		}
	}
	if(pos == 'row'){
...
function isExistDateDim(comp, tp){
	...
	if(tp == 'table'){
		...
		for(var i=0; i<comp.tableJson.cols.length; i++){
			if(comp.tableJson.cols[i].grouptype == 'date'){
				ret = true;
				break;
			}
		}
		...
		for(var i=0; i<comp.tableJson.rows.length; i++){
			if(comp.tableJson.rows[i].grouptype == 'date'){
				ret = true;
				break;
			}
		}
