...
function newdset(isupdate, dsetId){
	...
	$.ajax({
		...
		success: function(resp){
		   for(i=0; i<resp.length; i++){
			   var t = resp[i];
			   dsls = dsls + "<option value='"+t.id+"' "+(t.id == transform.dsid?"selected":"")+">"+t.dsname+"</option>";
		   }
		}
	});
		...
		buttons:[{
			...
			handler:function(){
				...
				$.ajax({
				   type: "POST",
				   async:false,
				   url: isupdate?"Dataset!update.action":"Dataset!save.action",
				   dataType:"HTML",
				   data: {cfg:JSON.stringify(transform), primary_table: transform.master, name:transform.name,dsid:transform.dsid, dsetId:transform.dsetId},
				   success: function(resp){
					   __hideLoading();
	...
	$("#pdailog #left2right").bind("click", function(){
		...
		if(isupdate){
			...
			$.ajax({
				type:'GET',
				url:'Dataset!listTableColumns.action',
				dataType:'json',
